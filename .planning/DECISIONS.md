...
- `.stamp-builder` is created after `docker build` succeeds, depends on `Dockerfile.builder` and `docker/entrypoint.sh`
- All lint and test targets depend on `$(BUILDER_STAMP)` — the builder auto-builds only when its inputs change
