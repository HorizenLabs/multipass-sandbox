...
if ! command -v yq &>/dev/null; then
    ...
    curl -fsSL "https://github.com/mikefarah/yq/releases/download/${YQ_VERSION}/${YQ_FILE}" -o /tmp/"${YQ_FILE}"
...
if ! command -v shellcheck &>/dev/null; then
    ...
    ARCH=$(uname -m)
    curl -fsSL "https://github.com/koalaman/shellcheck/releases/download/${SHELLCHECK_VERSION}/shellcheck-${SHELLCHECK_VERSION}.linux.${ARCH}.tar.xz" \
...
if ! command -v hadolint &>/dev/null; then
    ...
    curl -fsSL "https://github.com/hadolint/hadolint/releases/download/${HADOLINT_VERSION}/${HL_FILE}" -o /tmp/"${HL_FILE}"
...
if ! sudo -u ubuntu bash -c 'export HOME=/home/ubuntu; [ -s "$HOME/.nvm/nvm.sh" ]'; then
    echo "--- Installing nvm + Node.js + bun ---"
...
if ! sudo -u ubuntu bash -c 'export HOME=/home/ubuntu; export PATH="$HOME/.bun/bin:$PATH"; command -v crush &>/dev/null'; then
    echo "--- Installing AI assistants (crush, opencode, gemini, codex) ---"
    sudo -u ubuntu bash -c '
        export HOME=/home/ubuntu
        export PATH="$HOME/.bun/bin:$PATH"
...
if ! snap list multipass-sshfs &>/dev/null; then
    echo "--- Installing multipass-sshfs ---"
